 ...
 * Service that contain single Oracle session.
 ...
 * 3. Service Dependencies - we put its dependencies into its constructor.
  ...
  private def acquire(conf: OraServer): ZIO[Any,Exception,Connection] = for{
    ...
    connection <- ZIO.attemptBlocking {
      ...
      case e: Exception => ZIO.logError(e.getMessage) *>
        ZIO.fail(new Exception(s"createConnection : ${e.getMessage} - ${conf.getUrl()}"))
    }
  } yield connection
