...
/**
 * Delete rows and append new rows from ora to ch: 1) delete from ch.table where "where_filter" 2)
 * insert into ch.table select * from ora.table where "where_filter"
 *
...
/**
 * Only append new rows in ch which not exists (without delete). insert into ch.table select * from
 * ora.table where (c1,c2,...) not in (select distinct c1,c2,... from ch.table)
 */
...
/**
 * Only append new rows in ch which not exists (without delete). insert into ch.table select * from
 * ora.table where single_column > (select max(single_column) from ch.table)
 */
...
/**
 * In this case only used PRIMARY KEY FROM Clickhouse, look at sessCh.getPkColumns and update_fields
 * from json.
 */
